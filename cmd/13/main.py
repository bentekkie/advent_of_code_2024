...
with open(
    os.path.join(THIS_DIR, "..", "..", "pkg", "inputs", "puzzle_inputs", day, fname)
) as f:
    A = []
    B = []
    P = []
    for line in f.readlines():
        if line.startswith("Button A: "):
            A = [
                int(x)
                for x in line.removeprefix("Button A: ")
                .replace("X+", "")
                .replace("Y+", "")
                .split(", ")
            ]
            ...
            B = [
                int(x)
                for x in line.removeprefix("Button B: ")
                .replace("X+", "")
                .replace("Y+", "")
                .split(", ")
            ]
            ...
            P = [
                int(x)
                for x in line.removeprefix("Prize: ")
                .replace("X=", "")
                .replace("Y=", "")
                .split(", ")
            ]
...
def solve(A: list[int], B: list[int], P: list[int], bounds: tuple[int, int] | None = None):
    res = linprog(
        c=[3, 1],
        A_eq=list(zip(A, B)),
        b_eq=P,
